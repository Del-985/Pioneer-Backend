...
BEGIN
  ...
  IF journal_status IS NULL THEN
    -- When journal_entries is deleted, its ON DELETE CASCADE removes journal_lines.
    -- The parent row is no longer visible to this child-row trigger, so permit only
    -- that DELETE path. Inserts/updates with a missing parent remain invalid.
    IF TG_OP = 'DELETE' THEN
      RETURN OLD;
    END IF;
