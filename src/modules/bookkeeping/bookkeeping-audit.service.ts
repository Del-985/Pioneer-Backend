...
export async function listBookkeepingAudit(userId:string,q:AuditQuery){
  ...
  const result=await pool.query<Row>(
    `SELECT al.id,al.actor_user_id,u.display_name AS actor_name,al.legal_entity_id,al.business_unit_id,
            al.action,al.resource_type,al.resource_id,al.metadata,al.ip_address::text,al.user_agent,al.created_at
     FROM audit_log al LEFT JOIN users u ON u.id=al.actor_user_id LEFT JOIN business_units bu ON bu.id=al.business_unit_id
     WHERE al.action LIKE 'bookkeeping.%'
     ...
     ORDER BY al.created_at DESC LIMIT $10 OFFSET $11`,
    [userId,q.businessUnitId??null,q.legalEntityId??null,q.actorUserId??null,q.action??null,q.resourceType??null,q.resourceId??null,q.from??null,q.to??null,q.limit,q.offset]
  );
