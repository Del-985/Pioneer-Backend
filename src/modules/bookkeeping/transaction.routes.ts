...
async function presentTransaction(value: unknown) {
  return enrichTransactionIncomeCustomer(value);
}
...
bookkeepingTransactionRouter.patch('/:transactionId', async (req, res) => {
  const transactionId = requireRouteParam(req, 'transactionId');
  res.json({
    data: await presentTransaction(
      await updateBookkeepingTransaction(req.auth!.userId, transactionId, req.body)
    ),
  });
});
...
bookkeepingTransactionRouter.post('/:transactionId/post', async (req, res) => {
  ...
  if (transaction.status === 'posted') {
    res.json({ data: await presentTransaction(transaction) });
    return;
  }
  ...
  const result = await runIdempotent({
    ...
    key: normalizeIdempotencyKey(req.get('Idempotency-Key')),
    payload: input,
    execute: async () => presentTransaction(
      await postBookkeepingTransaction(req.auth!.userId, transactionId, input)
    ),
  });
  res.setHeader('Idempotency-Replayed', String(result.replayed));
  res.status(result.status).json({ data: result.value });
...
bookkeepingTransactionRouter.post('/:transactionId/void', async (req, res) => {
  ...
  if (transaction.status === 'posted') {
    ...
    const result = await runIdempotent({
      ...
      successStatus: 201,
      execute: async () => presentTransaction(
        await reverseBookkeepingTransaction(req.auth!.userId, transactionId, reversalInput)
      ),
    });
    res.setHeader('Idempotency-Replayed', String(result.replayed));
    res.status(result.status).json({ data: result.value });
  ...
  res.json({
    data: await presentTransaction(
      await voidBookkeepingTransaction(req.auth!.userId, transactionId, input)
    ),
  });
});
...
bookkeepingTransactionRouter.post('/:transactionId/reverse', async (req, res) => {
  ...
  if (transaction.status === 'reversed') {
    res.json({
      data: await presentTransaction({ transaction, reversalJournal: null }),
    });
    return;
  }
  ...
  const result = await runIdempotent({
    ...
    successStatus: 201,
    execute: async () => presentTransaction(
      await reverseBookkeepingTransaction(req.auth!.userId, transactionId, input)
    ),
  });
  res.setHeader('Idempotency-Replayed', String(result.replayed));
  res.status(result.status).json({ data: result.value });
