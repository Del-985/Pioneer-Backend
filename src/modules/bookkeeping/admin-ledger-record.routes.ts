...
import {
  createExpense, createRevenue, listExpenses, listRevenue,
...
adminLedgerRecordRouter.post('/expenses/:expenseId/post',requireAuth,async(req,res)=>{const businessUnitId=requireRouteParam(req,'businessUnitId');const expenseId=requireRouteParam(req,'expenseId');res.json({data:await postExpenseAtomic(req.auth!.userId,businessUnitId,expenseId,postExpenseSchema.parse(req.body))});});
...
adminLedgerRecordRouter.post('/revenue/:revenueId/post',requireAuth,async(req,res)=>{const businessUnitId=requireRouteParam(req,'businessUnitId');const revenueId=requireRouteParam(req,'revenueId');res.json({data:await postRevenueAtomic(req.auth!.userId,businessUnitId,revenueId,postRevenueSchema.parse(req.body))});});
